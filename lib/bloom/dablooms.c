...
*/
    bitmap->array = calloc(new_size, sizeof(char));
...
int bitmap_increment(bitmap_t *bitmap, unsigned int index, long offset)
{
	...
	// cout<<"\t index: "<<index<<" offset: "<<offset<<" access "<<access<<endl;
    // char output[9];
	// int v = bitmap->bytes;
    // while (v > 0)
    // {
		// std::string binary = std::bitset<8>(*bitmap->array).to_string(); //to binary
		// std::cout<<binary<<".";
        // --v;
    // }
	// cout<<endl;
	...
	// bool eh = false;
	// char output[9];
	// std::string binary = std::bitset<8>((bitmap->array[access])).to_string(); //to binary
	// if(binary!="00000000"){
		// eh = true;
		// std::cout<<binary<<".";
	// }
	...
	// if(eh){
		// binary = std::bitset<8>((bitmap->array[access])).to_string(); //to binary
...
counting_bloom_t *counting_bloom_init(unsigned int capacity, double error_rate, long offset)
{
	...
	// cout<<"bloom->nfuncs "<<bloom->nfuncs<<" bloom->counts_per_func "<<bloom->counts_per_func<<" bloom->size "<<bloom->size<<endl;
...
int counting_bloom_add(counting_bloom_t *bloom, const char *s, size_t len)
{
    ...
    // cout<<"Adding to Bloom for user id: "<< s<<endl;
    for (i = 0; i < bloom->nfuncs; i++) {
...
int counting_bloom_check(counting_bloom_t *bloom, const char *s, size_t len)
{
    ...
    hash_func(bloom, s, len, hashes);
	// cout<<"Checking Bloom for user id: "<< s <<" bloom->nfuncs "<< bloom->nfuncs<<" bloom->counts_per_func "<<bloom->counts_per_func<<endl;
