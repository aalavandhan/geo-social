...
public:
   ...
   : minimum_size(1),
     ...
     false_positive_probability(100.0 / projected_element_count),
...
public:
     ...
     desired_false_positive_probability_(p.false_positive_probability)
   {
	  ...
	  // cout<<"raw table size" <<raw_table_size_ <<" size of cell type: "<<sizeof(cell_type)<<endl;
   }
   ...
   virtual ~bloom_filter()
   {
	...
	inline void setBitsFrom (bloom_filter* B)
   {
		//take A and B and put all of set bits of B into A;
      /* intersection */
      if (
          (salt_count_  == B->salt_count_) &&
          (table_size_  == B->table_size_) &&
          (random_seed_ == B->random_seed_)
         )
      {
		inserted_element_count_+= B->inserted_element_count_;
		// cout<<"Raw Table Size of this: "<<raw_table_size_<<" and of B: "<<B->raw_table_size_<<endl;
        // for (std::size_t i = 0; i < raw_table_size_; ++i)
        // {
			// cout << std::bitset<bits_per_char>( bit_table_[i] );
        // }
		// cout<<"\n---------- & -----------"<<endl;
		// for (std::size_t i = 0; i < raw_table_size_; ++i)
        // {
			// cout << std::bitset<bits_per_char>( B->bit_table_[i] );

        // }
		// cout<<"\n--------output------------"<<endl;

		
		//AND THE TWO CHAR ARRAYS
		for (std::size_t i = 0; i < raw_table_size_; ++i)
        {
           bit_table_[i] |= B->bit_table_[i];
        }
	  ...
	  {
		cout<<"TRYING TO BITWISE-AND FILTERS WITH DIFFERENT SALT/TABLE_SIZE/RANDOM_SEED"<<endl;
	  }
   }
   ...
   inline bloom_filter& operator &= (const bloom_filter& f)
   {
      ...
      {
		inserted_element_count_+= f.inserted_element_count_;
		cout<<"Raw Table Size of left this: "<<raw_table_size_<<" and of right "<<f.raw_table_size_<<endl;
        for (std::size_t i = 0; i < raw_table_size_; ++i)
        {
			cout << std::bitset<bits_per_char>( bit_table_[i] );
        }
		...
		for (std::size_t i = 0; i < raw_table_size_; ++i)
        {
			cout << std::bitset<bits_per_char>( f.bit_table_[i] );
